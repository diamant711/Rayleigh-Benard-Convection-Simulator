...
private:
  ...
  */
  ...
  //functions
  static void
  m_d_solve (Eigen::Matrix<double, -1, 1> &, const Eigen::Matrix<int, 1, -1> &,
  ...
  m_spdiags (const Eigen::Matrix<Scalar, -1, -1> &,
             const Eigen::Matrix<int, -1, 1> &, size_t, size_t);
  template <typename Scalar>
  ...
  m_SparseToTriplet (Eigen::SparseMatrix<Scalar> &);
  template <typename Scalar>

  Eigen::Matrix<int, 1, -1> m_my_symamd (Eigen::Matrix<Scalar, -1, -1> &);

  void m_luP (Eigen::Matrix<double, -1, -1> &,
...
*/
void
RayBenConvection::m_d_solve (
    Eigen::Matrix<double, -1, 1> &mat, const Eigen::Matrix<int, 1, -1> &perm,
    const Eigen::PartialPivLU<Eigen::Matrix<double, -1, -1> > &L,
    const Eigen::PartialPivLU<Eigen::Matrix<double, -1, -1> > &U)
{
  mat (perm) = static_cast<Eigen::Matrix<double, -1, -1> > (
      ...
      if (delta_t > 0)
        {
          ...
          std::cerr << "INFO: RayBenConvection: m_ETA: eta = " << std::setw (4)
                    << std::setprecision (3) << simulation_state.eta
                    << "s\tspeed = " << simulation_state.velocity
                    << "cicle/s\tstep = " << n_calls << "/"
                    << simulation_state.total << std::endl;
          ::fflush (stdout);
        }
    }
...
template <typename Scalar>
Eigen::Matrix<Scalar, -1, -1>
RayBenConvection::m_spdiags (const Eigen::Matrix<Scalar, -1, -1> &B,
                             const Eigen::Matrix<int, -1, 1> &d, size_t m,
                             size_t n)
{
...
template <typename Scalar>
Eigen::Matrix<Scalar, -1, -1>
RayBenConvection::m_kron (const Eigen::Matrix<Scalar, -1, -1> &B,
...
template <typename Scalar>
std::vector<Eigen::Triplet<Scalar> >
RayBenConvection::m_SparseToTriplet (Eigen::SparseMatrix<Scalar> &A)
{
  std::vector<Eigen::Triplet<Scalar> > A_Triplet;
  for (int k = 0; k < A.outerSize (); ++k)
    {
      for (Eigen::InnerIterator<Eigen::SparseMatrix<Scalar> > it (A, k); it;
           ++it)
        {
          A_Triplet.push_back (
...
RayBenConvection::m_my_symamd (Eigen::Matrix<Scalar, -1, -1> &mat)
{
  ...
  retcode = symamd (mat.cols (), A, p, perm, (double *)NULL, stats, &calloc,
                    &free);
                         ...
                         std::vector<Eigen::Matrix<double, -1, -1> > &ret_vett)
{
  ...
  Eigen::Matrix<double, -1, -1> L_mat
      = LU_mat.triangularView<Eigen::UpLoType::UnitLower> ();
  Eigen::Matrix<double, -1, -1> U_mat
      = LU_mat.triangularView<Eigen::UpLoType::Upper> ();
  Eigen::Matrix<double, -1, -1> PL_mat
      = LU_dec.permutationP ().transpose () * L_mat;
...
RayBenConvection::init (unsigned int END_CICLE, double cold_temp,
                        double hot_temp, double Ray_numb, double Pr_numb,
                        double Re_numb)
{

  std::cerr << "INFO: RayBenConvection: init: Initializing simulation..."
            << std::endl;
  ...
  //IMPLICIT DIFFUSION:-
  //B.C vector(for u)
  m_bcu.setZero (), m_bcu.row (0).setConstant (2 * m_UW / std::pow (m_dx, 2));
  ...
  m_bcu (m_bcu.rows () - 1, 0)
      = 2 * m_UE / std::pow (m_dx, 2) + m_US / std::pow (m_dy, 2);
  m_bcu (0, (m_ny - 2) - 1)
      = 2 * m_UW / std::pow (m_dx, 2) + m_UN / std::pow (m_dy, 2);
  m_bcu (m_bcu.rows () - 1, m_bcu.cols () - 1)
      = 2 * m_UE / std::pow (m_dx, 2) + m_UN / std::pow (m_dy, 2);
  ...
  m_bcv (m_bcv.rows () - 1, 0)
      = 2 * m_VE / std::pow (m_dx, 2) + m_VS / std::pow (m_dy, 2);
  m_bcv (0, m_bcv.cols () - 1)
      = 2 * m_VW / std::pow (m_dx, 2) + m_VN / std::pow (m_dy, 2);
  m_bcv (m_bcv.rows () - 1, m_bcv.cols () - 1)
      = 2 * m_VE / std::pow (m_dx, 2) + m_VN / std::pow (m_dy, 2);
  ...
  m_Ax = m_spdiags (static_cast<Eigen::Matrix<double, -1, -1> > (
                        m_e * Eigen::Matrix<double, 1, 3> (1, -2, 1)),
                    Eigen::Matrix<int, 1, 3> (-1, 0, 1), m_nx - 1, m_nx - 1);

  m_Ay = m_spdiags (static_cast<Eigen::Matrix<double, -1, -1> > (
                        m_i * Eigen::Matrix<double, 1, 3> (1, -2, 1)),
                    Eigen::Matrix<int, 1, 3> (-1, 0, 1), m_ny - 2, m_ny - 2);

  m_Ax (0, 0) = -3;
  m_Ax (m_Ax.rows () - 1, m_Ax.rows () - 1) = -3;

  m_A = m_kron<double> (static_cast<Eigen::Matrix<double, -1, -1> > (
                            m_Ay / std::pow (m_dy, 2)),
                        Eigen::Matrix<double, m_nx - 1, m_nx - 1>::Identity ())
        + m_kron<double> (
            Eigen::Matrix<double, m_ny - 2, m_ny - 2>::Identity (),
            static_cast<Eigen::Matrix<double, -1, -1> > (
                m_Ax / std::pow (m_dx, 2)));

  m_Du = Eigen::Matrix<double, (m_nx - 1) * (m_ny - 2),
                       (m_nx - 1) * (m_ny - 2)>::Identity ()
         - m_dt * m_A / m_Re;
  ...
  m_Ax = m_spdiags (static_cast<Eigen::Matrix<double, -1, -1> > (
                        m_e * Eigen::Matrix<double, 1, 3> (1, -2, 1)),
                    Eigen::Matrix<int, 1, 3> (-1, 0, 1), m_nx - 2, m_nx - 2);

  m_Ay = m_spdiags (static_cast<Eigen::Matrix<double, -1, -1> > (
                        m_i * Eigen::Matrix<double, 1, 3> (1, -2, 1)),
                    Eigen::Matrix<int, 1, 3> (-1, 0, 1), m_ny - 1, m_ny - 1);
  ...
  m_Dv = Eigen::Matrix<double, (m_nx - 2) * (m_ny - 1),
                       (m_nx - 2) * (m_ny - 1)>::Identity ()
         - m_dt * m_A / m_Re;
  ...
  m_bcT (m_bcT.rows () - 1, m_bcT.cols () - 1)
      = m_TnE / m_dx + m_TN / std::pow (m_dy, 2);
  ...
  m_Tx = m_spdiags (static_cast<Eigen::Matrix<double, -1, -1> > (
                        m_e * Eigen::Matrix<double, 1, 3> (1, -2, 1)),
                    Eigen::Matrix<int, 1, 3> (-1, 0, 1), m_nx - 2, m_nx - 2);

  m_Ty = m_spdiags (static_cast<Eigen::Matrix<double, -1, -1> > (
                        m_i * Eigen::Matrix<double, 1, 3> (1, -2, 1)),
                    Eigen::Matrix<int, 1, 3> (-1, 0, 1), m_ny - 2, m_ny - 2);
  ...
  m_Tt
      = m_kron<double> (static_cast<Eigen::Matrix<double, -1, -1> > (
                            m_Ty / std::pow (m_dy, 2)),
                        Eigen::Matrix<double, m_nx - 2, m_nx - 2>::Identity ())
        + m_kron<double> (
            Eigen::Matrix<double, m_ny - 2, m_ny - 2>::Identity (),
  ...
  m_Tt = Eigen::Matrix<double, (m_nx - 2) * (m_ny - 2),
                       (m_nx - 2) * (m_ny - 2)>::Identity ()
         - m_dt * m_Tt / m_Pe;
  ...
  //%Boundary conditions
  m_T.row (0)
      = m_T.row (1) - Eigen::Matrix<double, 1, m_ny>::Constant (m_TnW * m_dx);
  m_T.row (m_T.rows () - 1)
      = m_T.row (m_T.rows () - 2)
        + Eigen::Matrix<double, 1, m_ny>::Constant (m_TnE * m_dx);
  m_T.col (0).setConstant (m_TS);
  m_T.col (m_T.cols () - 1).setConstant (m_TN);

  m_u.row (0)
      = Eigen::Matrix<double, 1, m_ny>::Constant (2 * m_UW) - m_u.row (1);
  m_u.row (m_u.rows () - 1)
      = Eigen::Matrix<double, 1, m_ny>::Constant (2 * m_UE)
        - m_u.row (m_u.rows () - 2);
  ...
  m_v.col (0)
      = Eigen::Matrix<double, m_nx, 1>::Constant (2 * m_VS) - m_v.col (1);
  m_v.col (m_v.cols () - 1)
      = Eigen::Matrix<double, m_nx, 1>::Constant (2 * m_VN)
        - m_v.col (m_v.cols () - 2);
  ...
  std::cerr << "INFO: RayBenConvection: init: Done! Time spent during "
...
RayBenConvection::simulation_state_t
RayBenConvection::eval_next_frame ()
{
  ...
  if (m_it < m_END_CICLE)
    {
      ...
      m_Tstar.block (1, 1, m_ii, m_jj)
          = (Tn.block (1, 1, m_ii, m_jj).array ()
             - (m_dt / m_dx / 2)
                   * (m_u.block (2, 1, m_ii, m_jj).array ()
                          * (Tn.block (1, 1, m_ii, m_jj)
                             + Tn.block (2, 1, m_ii, m_jj))
                                .array ()
                      - m_u.block (1, 1, m_ii, m_jj).array ()
                            * (Tn.block (1, 1, m_ii, m_jj)
                               + Tn.block (0, 1, m_ii, m_jj))
                                  .array ())
             - (m_dt / m_dy / 2)
                   * (m_v.block (1, 2, m_ii, m_jj).array ()
                          * (Tn.block (1, 1, m_ii, m_jj)
                             + Tn.block (1, 2, m_ii, m_jj))
                                .array ()
                      - m_v.block (1, 1, m_ii, m_jj).array ()
                            * (Tn.block (1, 1, m_ii, m_jj)
                               + Tn.block (1, 0, m_ii, m_jj))
                                  .array ()))
                .matrix ();
      ...
      t.col (0)
          = (m_Tstar.block (1, 1, m_Tstar.rows () - 2, m_Tstar.cols () - 2)
             + m_bcT)
                .reshaped (m_bcT.rows () * m_bcT.cols (), 1);

      t (m_pt) = static_cast<Eigen::Matrix<double, -1, -1> > (
          m_Ut_solver.solve (m_Lt_solver.solve (t (m_pt))));
      ...
      m_T.row (0) = m_T.row (1)
                    - Eigen::Matrix<double, 1, m_ny>::Constant (m_TnW * m_dx);
      m_T.row (m_T.rows () - 1)
          = m_T.row (m_T.rows () - 2)
            + Eigen::Matrix<double, 1, m_ny>::Constant (m_TnE * m_dx);
      ...
      m_uhalf.block (2, 1, m_ii, m_jj)
          = (0.5
                 * (un.block (3, 1, m_ii, m_jj) + un.block (1, 1, m_ii, m_jj))
                       .array ()
             - (m_dt / m_dx / 8)
                   * (un.block (3, 1, m_ii, m_jj)
                      + un.block (2, 1, m_ii, m_jj))
                         .array ()
                         .square ()
             - (un.block (2, 1, m_ii, m_jj) + un.block (1, 1, m_ii, m_jj))
                   .array ()
                   .square ()
             - (m_dt / m_dy / 8)
                   * ((un.block (2, 1, m_ii, m_jj)
                       + un.block (2, 2, m_ii, m_jj))
                              .array ()
                          * (vn.block (1, 2, m_ii, m_jj)
                             + vn.block (2, 2, m_ii, m_jj))
                                .array ()
                      - (un.block (2, 1, m_ii, m_jj)
                         + un.block (2, 0, m_ii, m_jj))
                                .array ()
                            * (vn.block (1, 1, m_ii, m_jj)
                               + vn.block (2, 1, m_ii, m_jj))
                                  .array ()))
                .matrix ();
      m_vhalf.block (1, 2, m_ii, m_jj)
          = (0.5
                 * (vn.block (1, 3, m_ii, m_jj) + vn.block (1, 1, m_ii, m_jj))
                       .array ()
             - (m_dt / m_dx / 8)
                   * ((un.block (2, 1, m_ii, m_jj)
                       + un.block (2, 2, m_ii, m_jj))
                          ...
                          * (vn.block (1, 2, m_ii, m_jj)
                             + vn.block (2, 2, m_ii, m_jj))
                                .array ()
                      - (un.block (1, 1, m_ii, m_jj)
                         + un.block (1, 0, m_ii, m_jj))
                                .array ()
                            * (vn.block (1, 2, m_ii, m_jj)
                               + vn.block (0, 2, m_ii, m_jj))
                                  .array ())
             - (m_dt / m_dy / 8)
                   * ((vn.block (1, 3, m_ii, m_jj)
                       + vn.block (1, 2, m_ii, m_jj))
                          .array ()
                          .square ()
                      - (vn.block (1, 2, m_ii, m_jj)
      ...
      m_uconv.block (2, 1, m_ii, m_jj)
          = (un.block (2, 1, m_ii, m_jj).array ()
             - (m_dt / (4 * m_dx))
                   * ((m_uhalf.block (3, 1, m_ii, m_jj)
                       + m_uhalf.block (2, 1, m_ii, m_jj))
                          .array ()
                          .square ()
                      - (m_uhalf.block (2, 1, m_ii, m_jj)
                         + m_uhalf.block (1, 1, m_ii, m_jj))
                            .array ()
                            .square ())
             - (m_dt / (4 * m_dy))
                   * ((m_uhalf.block (2, 1, m_ii, m_jj)
                       + m_uhalf.block (2, 2, m_ii, m_jj))
                              .array ()
                          * (m_vhalf.block (1, 2, m_ii, m_jj)
                             + m_vhalf.block (2, 2, m_ii, m_jj))
                                .array ()
                      - (m_uhalf.block (2, 1, m_ii, m_jj)
                         + m_uhalf.block (2, 0, m_ii, m_jj))
                                .array ()
                            * (m_vhalf.block (1, 1, m_ii, m_jj)
                               + m_vhalf.block (2, 1, m_ii, m_jj))
                                  .array ()))
                .matrix ();
      m_vconv.block (1, 2, m_ii, m_jj)
          = (vn.block (1, 2, m_ii, m_jj).array ()
             - (m_dt / (4 * m_dx))
                   * ((m_uhalf.block (2, 1, m_ii, m_jj)
                       + m_uhalf.block (2, 2, m_ii, m_jj))
                              .array ()
                          * (m_vhalf.block (1, 2, m_ii, m_jj)
                             + m_vhalf.block (2, 2, m_ii, m_jj))
                                .array ()
                      - (m_uhalf.block (1, 1, m_ii, m_jj)
                         + m_uhalf.block (1, 0, m_ii, m_jj))
                                .array ()
                            * (m_vhalf.block (1, 2, m_ii, m_jj)
                               + m_vhalf.block (0, 2, m_ii, m_jj))
                                  .array ())
             - (m_dt / (4 * m_dy))
                   * ((m_vhalf.block (1, 3, m_ii, m_jj)
                       + m_vhalf.block (1, 2, m_ii, m_jj))
                          .array ()
                          .square ()
                      - (m_vhalf.block (1, 2, m_ii, m_jj)
      ...
      m_vconv.block (1, 2, m_ii, m_jj)
          = m_vconv.block (1, 2, m_ii, m_jj)
            + (m_Gr / std::pow (m_Re, 2))
                  * (0.5
                         * (m_T.block (1, 1, m_ii, m_jj)
                            + m_T.block (1, 2, m_ii, m_jj))
                     - Eigen::Matrix<double, m_ii, m_jj>::Constant (m_To));
      ...
      U.col (0)
          = (m_uconv.block (1, 1, m_uconv.rows () - 2, m_uconv.cols () - 2)
             + m_bcu)
                .reshaped (m_bcu.rows () * m_bcu.cols (), 1);

      Eigen::Matrix<double, -1, 1> V;
      V.resize (m_bcv.rows () * m_bcv.cols (), 1);

      V.col (0)
          = (m_vconv.block (1, 1, m_vconv.rows () - 2, m_vconv.cols () - 2)
             + m_bcv)
                .reshaped (m_bcv.rows () * m_bcv.cols (), 1);

      std::thread t1 (m_d_solve, std::ref (U), std::ref (m_pu),
                      std::ref (m_Lu_solver), std::ref (m_Uu_solver));
      std::thread t2 (m_d_solve, std::ref (V), std::ref (m_pv),
                      std::ref (m_Lv_solver), std::ref (m_Uv_solver));
      ...
      m_S.block (1, 1, m_ii, m_jj) = (m_ustar.block (2, 1, m_ii, m_jj)
                                      - m_ustar.block (1, 1, m_ii, m_jj))
                                         / m_dx
                                     + (m_vstar.block (1, 2, m_ii, m_jj)
                                        - m_vstar.block (1, 1, m_ii, m_jj))
                                           / m_dy;

      Eigen::Matrix<double, -1, 1> s;
      s.resize ((m_S.rows () - 2) * (m_S.cols () - 2), 1);

      s.col (0) = m_S.block (1, 1, m_S.rows () - 2, m_S.cols () - 2)
                      .reshaped ((m_S.rows () - 2) * (m_S.cols () - 2), 1);

      s (m_pp) = static_cast<Eigen::Matrix<double, -1, -1> > (
          m_Up_solver.solve (m_Lp_solver.solve (s (m_pp))));
      ...
      m_u.block (2, 1, m_ii, m_jj)
          = m_ustar.block (2, 1, m_ii, m_jj)
            - (m_p.block (2, 1, m_ii, m_jj) - m_p.block (1, 1, m_ii, m_jj))
                  / m_dx;
      m_v.block (1, 2, m_ii, m_jj)
          = m_vstar.block (1, 2, m_ii, m_jj)
            - (m_p.block (1, 2, m_ii, m_jj) - m_p.block (1, 1, m_ii, m_jj))
                  / m_dy;
...
void
RayBenConvection::write_current_data ()
{
  if (m_it == 0)
    {
      m_output_header_file.open (OUTPUT_HEADER_FILE_NAME, std::ios_base::out);
      std::cerr
          << "INFO: RayBenConvection: m_write_current_data: opened data file"
          << std::endl;
      if (!m_output_header_file.is_open ())
        {
          std::cerr << "ERROR: RayBenConvection: Failed opening data file. "
                       "Exiting..."
                    << std::endl;
          return;
        }
      m_output_header_file << "#define NX " << m_nx
                           ...
                           << m_END_CICLE
                           << "\n"
                              "const double cold_temp = "
                           << m_TN
                           << ";\n"
                              "const double hot_temp = "
                           << m_TS
                           << ";\n"
  ...
  else if (m_it == m_END_CICLE)
    {
      ...
      std::cerr
          << "INFO: RayBenConvection: m_write_current_data: closed data file"
          << std::endl;
    }
  else
    {
