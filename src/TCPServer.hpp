...
TCPServer::TCPServer (std::shared_ptr<boost::asio::io_context> executor_ptr,
                      int port)
    : Server (executor_ptr),
      m_acceptor (m_get_executor (), boost::asio::ip::tcp::endpoint (
                            ...
                            const boost::system::error_code &error)
{
  ...
  if (!error)
    {
      new_connection.remote_ip = new_connection.connection_ptr->get_socket ()
                                     .remote_endpoint ()
                                     .address ();
      new_connection.port = new_connection.connection_ptr->get_socket ()
                                .remote_endpoint ()
                                .port ();
...
TCPServer::m_start_accept (void)
{
  ...
  m_acceptor.async_accept (new_connection.connection_ptr->get_socket (),
                           boost::bind (&TCPServer::m_handle_accept, this,
                                        new_connection,
